    ...
    if (!globalThis.fs) {
        ...
        global.fs.writeSync = function (fd, buf) {
            if (fd === 1 || fd === 2) {
                ...
                let s = outputBuf.split('\n').filter(x => x.length > 0).pop();
                //const nl = outputBuf.lastIndexOf("\n");
                //if (nl != -1) outputBuf = outputBuf.substr(nl + 1);
                //console.log(`[ fs.writeSync ] `, buf.length, s.length, s);
                //console.log('arguments =', arguments)
                global.fs.messages.push(s);
                return buf.length;
            } else {
        ...
        globalThis.fs = {
            ...
            writeSync(fd, buf) {
                outputBuf += decoder.decode(buf);
                const nl = outputBuf.lastIndexOf("\n");
                console.log('[ writeSync ] outputBuf =', outputBuf);
                if (nl != -1) {
